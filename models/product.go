...
func SearchProducts(db *sql.DB, query string) ([]Product, error) {
	query = "%" + query + "%"
	rows, err := db.Query("SELECT id, name, description, price, image_url FROM products WHERE name ILIKE $1 OR description ILIKE $2", query, query)
	if err != nil {
		return nil, err
	}
	defer rows.Close()
...
func GetProductByID(db *sql.DB, id int) (*Product, error) {
	query := "SELECT id, name, description, price, image_url FROM products WHERE id = $1"
...
func InsertProduct(db *sql.DB, product Product) (int, error) {
	...
	var id int
	err := db.QueryRow(query, product.Name, product.Description, product.Price, product.ImageURL).Scan(&id)
	if err != nil {
		return 0, err
	}
